...
export default function POSPage() {
  ...
  const printReceipt = () => {
    ...
    if (win) {
      win.document.write(`<html><body style="font-family:monospace; width:300px; padding:20px;"><center><h2>VELKARA.</h2></center><hr/><p>Total: ৳${finalTotal}</p><script>window.print(); window.close();</script></body></html>`);
      win.document.close();
    }
  };
    ...
    <div className="min-h-screen bg-[#f8fafc] p-4 md:p-10">
      ...
      <div className="max-w-7xl mx-auto flex justify-between items-center mb-8">
        <button 
          ...
          className="flex items-center gap-2 bg-white px-5 py-3 rounded-2xl font-black text-slate-600 shadow-sm border border-slate-100 hover:text-blue-600 transition-all"
        >
          <ArrowLeft size={20} /> Dashboard
        </button>
        <button 
          onClick={() => window.location.href = '/scanner'}
          className="flex items-center gap-2 bg-slate-900 text-white px-5 py-3 rounded-2xl font-black shadow-lg hover:scale-105 transition-all"
        >
      ...
      <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1 space-y-6">
          <div className="bg-white p-8 rounded-[40px] shadow-xl border border-white relative z-50">
            ...
            <div className="relative">
              <input 
                className="w-full bg-slate-100 pl-6 pr-5 py-5 rounded-2xl outline-none focus:ring-4 ring-blue-50 font-bold text-lg" 
                ...
                <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-[25px] shadow-2xl border border-slate-100 overflow-hidden">
                  {searchResults.map(product => (
                    <div key={product.id} onClick={() => addToCartFromSearch(product)} className="p-5 hover:bg-blue-50 cursor-pointer flex justify-between items-center border-b border-slate-50 last:border-none">
                      <div><p className="font-black">{product.name}</p><p className="text-[10px] text-blue-500 font-bold">{product.sku}</p></div>
                ...
                {cart.map((item) => (
                  <tr key={item.cartId} className="hover:bg-slate-50/50">
                    <td className="px-8 py-5">
                      ...
                      <p className="text-[10px] text-slate-400 font-black">{item.variant_size} / {item.variant_color}</p>
                    </td>
                    <td className="px-8 py-5">
        ...
        <div className="w-full lg:w-[420px] space-y-6">
          ...
          <div className="bg-slate-900 text-white p-10 rounded-[45px] shadow-2xl relative overflow-hidden text-center">
            <p className="text-slate-500 font-black text-[10px] uppercase tracking-widest mb-2">Grand Total</p>
            <p className="text-6xl font-black mb-10 tracking-tighter">৳{finalTotal}</p>
            <button 
              onClick={handleCompleteSale} 
              disabled={loading} 
              className="w-full bg-blue-600 hover:bg-blue-500 py-6 rounded-[28px] font-black text-xl flex justify-center items-center shadow-xl"
            >
